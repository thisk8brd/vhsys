...
const initialState = {
  ...
  subtitle: 'Confira nossas principais funcionalidades para você controlar sua empresa',
  description: 'O software de gestão VHSYS existe para ajudar micro e pequenas empresas a terem mais controle sobre o negócio e praticidade no dia a dia. De forma integrada, o sistema de gestão abrange as áreas de vendas, financeiro, estoque e emissão de notas fiscais eletrônicas.',
...
function Services(props) {
  ...
  const renderServiceItem = (item) => (
    <li className="inner__item">{item}</li>
  );
    ...
    <section className="services">
      <div className="container services__container">
        ...
        <ul className="services__list list">
          {content.services && content.services.length > 0 ? content.services.map(service => renderService(service)) : null}
        </ul>
      </div>
    </section>
  )
