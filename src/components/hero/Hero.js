...
const content = {
  ...
  text: 'O VHSYS é o Sistema de Gestão Empresarial Online que integra todas as áreas da sua empresa. Com ele você têm:',
  list: [
...
function Hero(props) {
  ...
  const renderBannerItem = (item) => {
    return <li className="banner-list__item">
        ...
        <div className="hero__content">
          ...
          <div className="hero__banner-info">
            <h1 className="heading heading-1 hero__heading">
              ...
              <span><span className="is--green">Experimente Grátis</span> por 7 dias!</span>
            </h1>
            <Gap height={'4rem'} />
            ...
            <ul className="hero__banner-list banner-list">
              {content && content.list && content.list.length > 0 ? content.list.map(item => renderBannerItem(item)) : null}
            </ul>
          </div>
        </div>
        <HeroForm/>
